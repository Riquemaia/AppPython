...
layout = [[sg.Col(logo, element_justification='c')],
...
window = sg.Window('Maia generator', layout)
    ...
    if event in (sg.WIN_CLOSED, 'Cancel'):
        ...
        try:
            ...
            for z in range(len(column_names)):
                window.extend_layout(window['k_col_type'], [[sg.Push(), sg.T('{}'.format(column_names[z]), key=f'k_col_type_nam{z}'), sg.Combo(values=types, default_value='Selecione o tipo do dado...', size=(25,6), readonly=True, enable_events=True, key=f'k_col_type_val{z}'), sg.In(key=f'k_col_type_fmt{z}')]])
